...
export function CustomSelect(props) {
  ...
  const urlFetch = field.properties?.url ?? 'http://universities.hipolabs.com/search'

  const onScrollEnd = async () => {
    if (scrollLocked || !urlFetch) return
    const response = await fetch(`${urlFetch}?search=${search}&page=${currentPage + 1}&size=10`)
  ...
  return html`<div ref=${componentRef} class=${formFieldClasses(customSelectType, { errors, disabled, readonly })}>
    ...
    <div class="fjs-input-group">
      ...
      html`<ul onScroll=${onScroll} className="options-list">
        ${options.map(
          (option) =>
            html`<li key=${option.key} class="option-item" onClick=${() => handleSelect(option)}>${option.value}</li>`,
        )}
      </ul>`}
    </div>
...
CustomSelect.config = {
  ...
  properties: {
    urlTemplate: {
      type: 'String',
      label: 'URL Template',
      description: 'Supports fx expressions. Use {value} as placeholder.',
      feel: true // Enables the fx (expression) toggle in the modeler
    }
  },
  ...
  propertiesPanelEntries: [
    ...
    'disabled',
    'readonly',
    'urlTemplate' // Include your custom field here
  ]
}
